...
class OwnedView(discord.ui.View):
    """Base View that restricts interactions to the original invoker."""

    def __init__(self, *, owner_id: int, timeout: float = VIEW_TIMEOUT):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.owner_id:
            await interaction.response.send_message(
                "⛔ Only the user who opened this panel can interact.",
                ephemeral=True,
            )
            return False
        return True
...
class UserManagementView(OwnedView):
    """View used for managing individual guild members."""

    def __init__(self, cog: "Management", owner_id: int):
        super().__init__(owner_id=owner_id, timeout=VIEW_TIMEOUT)
        self.cog = cog
    ...
    async def special_tag_btn(self, inter: discord.Interaction, button: discord.ui.Button):
        ...
        role = inter.guild.get_role(special_role_id) if special_role_id else None
        if not role:
            return await inter.response.send_message("❌ Special role not found on server.", ephemeral=True)
        ...
        try:
            ...
            await inter.response.send_message(
                "❌ I don't have permission to manage roles. Check my role hierarchy.", ephemeral=True
            )
    ...
    async def back_button(self, inter: discord.Interaction, button: discord.ui.Button):
        await inter.response.edit_message(
            embed=self.cog.get_main_embed(), view=ManagementView(self.cog, owner_id=self.owner_id)
        )
        self.stop()


class ManagementView(OwnedView):
    ...
    async def _dropdown_callback(self, inter: discord.Interaction):
        ...
        match self.dropdown.values[0]:
            case "User Management":
                ...
                target_view = UserManagementView(self.cog, owner_id=self.owner_id)
...
class Management(commands.Cog):
    ...
    def _get_last_logs(self, lines: int = 12) -> str:
        ...
        return self.log_stream.get_recent_lines(lines)
    ...
    async def management(self, interaction: discord.Interaction):
        ...
        if not await self.bot.has_access(interaction.user, interaction.guild):
            role = interaction.guild.get_role(guild_config.master_role_id) if guild_config else None
            master_name = role.name if role else "Master"
            embed = discord.Embed(
                title="⛔ Permission Denied",
                description=f"You need the `{master_name}` role to manage the bot.",
                color=self.embed_colors.get("error", discord.Color.red()),
            )
            return await interaction.response.send_message(embed=embed, ephemeral=True)

        await interaction.response.send_message(
            embed=self.get_main_embed(), view=ManagementView(self, owner_id=interaction.user.id), ephemeral=True
        )
