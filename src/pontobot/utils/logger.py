...
class DequeLogHandler(logging.Handler):
    ...
    def get_recent_lines(self, count: int = 12) -> str:
        """Returns the last N formatted log lines efficiently."""
        recent = list(islice(reversed(self.buffer), count))
        recent.reverse()
        return "\n".join(recent) if recent else "No logs recorded yet."
