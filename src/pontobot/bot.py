...
class PontoBot(commands.Bot):
    ...
    def __init__(self, db_path: str = "bot.db", sync_on_startup: bool | None = None):
        setup_logging()
        ...
        if sync_on_startup is None:
            sync_on_startup = os.environ.get("SYNC_COMMANDS", "false").lower() in ("true", "1", "yes")
    ...
    async def has_access(self, member: discord.Member, guild: discord.Guild) -> bool:
        ...
        return any(role.id == guild_data.master_role_id for role in member.roles)
        ...
        for path in cogs_dir.glob("*.py"):
            if not path.name.startswith("_"):
                try:
                    ...
                    logger.info("📦 Extension loaded successfully: %s", path.stem)
                except Exception as e:
                    logger.error("❌ Failed to load extension %s: %s", path.stem, e)

        if self.sync_on_startup:
            try:
                logger.info("🔄 Syncing global application commands...")
                synced = await self.tree.sync()
                logger.info("✅ %d global slash commands synced successfully.", len(synced))
            except Exception as e:
                logger.error("❌ Error syncing global slash commands: %s", e)
        else:
            logger.info("⏩ Skipping global command sync (set SYNC_COMMANDS=true to force sync).")
    ...
    async def on_app_command_error(self, interaction: discord.Interaction, error: app_commands.AppCommandError) -> None:
        ...
        logger.error("Error in slash command '%s': %s", interaction.command, error)
